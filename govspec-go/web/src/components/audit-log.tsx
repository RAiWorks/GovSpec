...
export function AuditLog({ featureId }: { featureId?: string }) {
  ...
  useEffect(() => {
    api.audit.list(featureId)
      .then(setLogs)
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [featureId]);

  if (loading) return <div className="text-center py-8 text-muted-foreground">Loading audit log...</div>;
  if (logs.length === 0) return <div className="text-center py-8 text-muted-foreground">No audit entries yet.</div>;
        ...
        <TableBody>
          {logs.map((log) => (
            <TableRow key={log.id}>
              ...
              {!featureId && <TableCell>{log.featureName || '—'}</TableCell>}
              <TableCell>{log.fromStatus ? <StatusBadge status={log.fromStatus} /> : '—'}</TableCell>
              <TableCell className="text-muted-foreground">→</TableCell>
              <TableCell><StatusBadge status={log.toStatus} /></TableCell>
